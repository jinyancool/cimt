...
rawobj <- read_rds(glue("{out_put_dir}/newadd_test2.rds")) %>% dplyr::filter(!XINGMING %in% c("徐利虎", "徐中凯"))
...
pdf(glue("{out_dir_point}/add_data_point2.pdf"),width = 6, height = 5)
print(
  pp1
)
dev.off()
...
pp3 <- ggplot(pddf %>% dplyr::filter(gender == 0), aes(x = preidct, y = true)) + 
  ...
  theme_bmbdc(font_size = 16, fill = "white")
pdf(glue("{out_dir_point}/add_data_point_gender2.pdf"),width = 4.5, height = 4)
print(
  pp2
)
print(
...
dev.off()
write_rds(list(total = pp1, male = pp2, female = pp3), file = glue("{out_dir_point}/add_data_point_oldsplit2.rds"))
